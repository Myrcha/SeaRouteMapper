...
function createLayers() {
	...
	sealayer = new ol.layer.VectorTile({
		...
		source: new ol.source.VectorTile({
			url: 'https://api.maptiler.com/tiles/land/{z}/{x}/{y}.pbf?key=' + sealayerKey,
			tileSize: 512,
			maxZoom: 14,
...
function calculateGreatCircleRoute(startLon, startLat, endLon, endLat, style) {
	...
	points.push({
		lon: toDegrees(startLonRad),
		lat: toDegrees(startLatRad)
	});
	midpointsIterator(0, startLonRad, startLatRad, endLonRad, endLatRad);
	points.push({
		lon: toDegrees(endLonRad),
		lat: toDegrees(endLatRad)
	});
...
function showDialog() {
	...
	submitButton.onclick = async function() {
		let lonlat = validateInputs(input.value, input2.value, selectNS.value, selectEW.value);
		let lon = lonlat[0];
		let lat = lonlat[1];
